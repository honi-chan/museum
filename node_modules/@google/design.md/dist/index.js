...
function requirePicocolors() {
  ...
  let formatter2 = (open, close2, replace = open) => (input) => {
    let string3 = "" + input, index2 = string3.indexOf(close2, open.length);
    return ~index2 ? open + replaceClose2(string3, close2, replace, index2) + close2 : open + string3 + close2;
  };
  let replaceClose2 = (string3, close2, replace, index2) => {
    let result = "", cursor = 0;
    do {
      result += string3.substring(cursor, index2) + replace;
      cursor = index2 + close2.length;
      index2 = string3.indexOf(close2, cursor);
    } while (~index2);
    return result + string3.substring(cursor);
  };
  let createColors2 = (enabled = isColorSupported2) => {
    let f2 = enabled ? formatter2 : () => String;
    return {
...
async function prompt(message, opts = {}) {
  const handleCancel = (value2) => {
    if (typeof value2 !== "symbol" || value2.toString() !== "Symbol(clack:cancel)") {
      return value2;
    }
    switch (opts.cancel) {
      case "reject": {
...
var NEVER = INVALID;
// src/linter/parser/spec.ts
var ParserInputSchema = exports_external.object({
  content: exports_external.string().min(1, "Content must not be empty")
});
var ParserErrorCode = exports_external.enum([
  "EMPTY_CONTENT",
  "NO_YAML_FOUND",
  "YAML_PARSE_ERROR",
  "DUPLICATE_SECTION",
...
var TypeDefSchema = exports_external.object({
  description: exports_external.string(),
  formats: exports_external.array(exports_external.string()).optional(),
  units: exports_external.array(exports_external.string()).optional(),
  note: exports_external.string().optional(),
  recommendation: exports_external.string().optional()
});
var ConfigSchema = exports_external.object({
  version: exports_external.string(),
  limits: exports_external.object({
    max_token_nesting_depth: exports_external.number().default(20),
    max_reference_depth: exports_external.number().default(10)
  }).default({}),
...
var MAX_TOKEN_NESTING_DEPTH = config.limits.max_token_nesting_depth;
var MAX_REFERENCE_DEPTH = config.limits.max_reference_depth;
var STANDARD_UNITS = config.units;
var SPEC_TYPES = config.types;
...
var PRIMITIVE_TYPES = config.types;
...
var MAX_COLOR_MIX_DEPTH = 32;
function parseCssColor(colorStr, depth = 0) {
...
var VALID_TYPOGRAPHY_PROPS2 = VALID_TYPOGRAPHY_PROPS;
...
var SCHEMA_KEY_SET = new Set(SCHEMA_KEYS);
var TYPOGRAPHY_PROP_SET = new Set(VALID_TYPOGRAPHY_PROPS2);
...
class ModelHandler {
  execute(input) {
    try {
      ...
      for (const [key, value2] of symbolTable) {
        if (typeof value2 !== "string" || !isTokenReference(value2))
          continue;
        const resolved = resolveReference(symbolTable, value2.slice(1, -1), new Set);
...
function buildCollisionGuard(category, findings) {
...
function parseDimension(raw) {
  if (typeof raw !== "string") {
    throw new Error(`parseDimension expected a string, got ${typeof raw}. ` + `This usually indicates a malformed token in your design file.`);
  }
  const value2 = raw.trim();
  if (value2 === "") {
    throw new Error('parseDimension received an empty string. Please provide a valid dimension (e.g., "16px", "1.5rem").');
  }
  const parts = parseDimensionParts(value2);
  if (!parts) {
  ...
  for (const key of Object.keys(props)) {
    if (!TYPOGRAPHY_PROP_SET.has(key)) {
      findings.push({
        severity: "warning",
        path: `${path}.${key}`,
        message: `'${key}' is not a recognized typography property. Valid properties: ${VALID_TYPOGRAPHY_PROPS2.join(", ")}.`
      });
    }
  }
  return result;
...
function forEachLeaf(obj, fn, prefix = "", depth = 0, findings, rootPath) {
  ...
  for (const [key, value2] of Object.entries(obj)) {
    ...
    if (value2 !== null && typeof value2 === "object" && !Array.isArray(value2)) {
      forEachLeaf(value2, fn, fullPath, depth + 1, findings, rootPath);
    } else {
      fn(fullPath, value2);
    }
  }
...
// src/linter/linter/rules/levenshtein.ts
...
// src/linter/linter/rules/unknown-key.ts
var MAX_TYPO_DISTANCE = 2;
function unknownKey(state) {
  ...
  return (state.unknownKeys ?? []).flatMap((key) => {
    ...
    for (const known of SCHEMA_KEYS) {
      if (Math.abs(key.length - known.length) > MAX_TYPO_DISTANCE)
        continue;
...
// src/linter/linter/rules/token-like-ignored.ts
var HEX_COLOR_RE = /^#([0-9a-fA-F]{3,4}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$/;
var CSS_DIMENSION_RE = /^-?\d*\.?\d+[a-zA-Z%]+$/;
var MAX_TOKEN_VALUE_LENGTH = 64;
var TYPOGRAPHY_PROPS = new Set(["fontFamily", "fontSize", "fontWeight", "lineHeight", "letterSpacing"]);
function isTokenLikeMap(value2) {
  if (value2 === null || typeof value2 !== "object" || Array.isArray(value2)) {
    return false;
  }
  const obj = value2;
...
function hasTokenLikeContent(obj) {
  for (const [key, val] of Object.entries(obj)) {
    ...
    if (typeof val === "string") {
      if (val.length <= MAX_TOKEN_VALUE_LENGTH && (HEX_COLOR_RE.test(val) || CSS_DIMENSION_RE.test(val)))
        return true;
...
function tokenLikeIgnored(state) {
  const unknownKeys = state.unknownKeys ?? [];
  const unknownKeyValues = state.unknownKeyValues ?? {};
  const findings = [];
  for (const key of unknownKeys) {
    const value2 = unknownKeyValues[key];
    if (isTokenLikeMap(value2)) {
      findings.push({
        path: key,
        message: `"${key}" looks like a design-token map but is not a recognized schema key ` + `(colors, typography, spacing, rounded, components). ` + `It will be silently ignored by export commands. ` + `Rename it to a supported key or move its values under a recognized section.`
      });
    }
  }
  return findings;
}
var tokenLikeIgnoredRule = {
  name: "token-like-ignored",
  severity: "warning",
  description: "Warns when a top-level YAML key looks like a design-token map but is not " + "part of the recognized export schema and will be silently ignored.",
  run: tokenLikeIgnored
};

// src/linter/linter/rules/omitted.ts
function omittedRules(state) {
  const findings = [];
  if (!state.omitted)
    return findings;
  ...
  for (const item of state.omitted) {
    ...
    let isPresent = false;
    if (sectionName === "colors" && state.colors && state.colors.size > 0)
      isPresent = true;
    if (sectionName === "typography" && state.typography && state.typography.size > 0)
      isPresent = true;
    if (sectionName === "spacing" && state.spacing && state.spacing.size > 0)
      isPresent = true;
    if (sectionName === "rounded" && state.rounded && state.rounded.size > 0)
      isPresent = true;
    if (sectionName === "components" && state.components && state.components.size > 0)
      isPresent = true;
    if (isPresent) {
      findings.push({
        severity: "warning",
...
var VALID_TOKEN_NAME = /^[a-zA-Z0-9][a-zA-Z0-9-]*$/;
...
// src/linter/css-vars/handler.ts
class CssVarsEmitterHandler {
  execute(state) {
    const declarations = [];
    for (const [name, color2] of state.colors) {
      declarations.push({
        name: `color-${this.cssSafe(name)}`,
        value: color2.hex.toLowerCase()
      });
    }
  ...
  mapDimensionGroup(declarations, group, dims) {
    for (const [name, dim] of dims) {
      declarations.push({
...
// src/linter/css-vars/serialize.ts
function serializeCssVars(declarations, options = {}) {
  const variablePrefix = options.prefix ? `${options.prefix}-` : "";
  const lines = declarations.map((declaration) => `  --${variablePrefix}${declaration.name}: ${declaration.value};`);
  if (lines.length === 0)
    return `:root {
}
`;
...
function _defu(baseObject, defaults, namespace = ".", merger) {
  ...
  for (const key in baseObject) {
    ...
    } else if (isPlainObject$1(value2) && isPlainObject$1(object[key])) {
      object[key] = _defu(value2, object[key], (namespace ? `${namespace}.` : "") + key.toString(), merger);
    } else {
      object[key] = value2;
    }
  }
...
function replaceClose(index2, string3, close2, replace, head = string3.slice(0, Math.max(0, index2)) + replace, tail = string3.slice(Math.max(0, index2 + close2.length)), next = tail.indexOf(close2)) {
  return head + (next < 0 ? tail : replaceClose(next, tail, close2, replace));
}
function clearBleed(index2, string3, open, close2, replace) {
  return index2 < 0 ? open + string3 + close2 : open + replaceClose(index2, string3, close2, replace) + close2;
}
function filterEmpty(open, close2, replace = open, at = open.length + 1) {
  return (string3) => string3 || !(string3 === "" || string3 === undefined) ? clearBleed(("" + string3).indexOf(close2, at), string3, open, close2, replace) : "";
}
function init(open, close2, replace) {
  return filterEmpty(`\x1B[${open}m`, `\x1B[${close2}m`, replace);
}
var colorDefs = {
  reset: init(0, 0),
...
function stringWidth$1(string3, options = {}) {
...
async function readInput(filePath, stdin2 = process.stdin) {
  if (filePath === "-") {
    if (stdin2.isTTY) {
...
function isLintOutput(obj) {
  if (!Array.isArray(obj.findings))
    return false;
  if (typeof obj.summary !== "object" || obj.summary === null)
    return false;
  const s2 = obj.summary;
  return typeof s2.errors === "number" && typeof s2.warnings === "number" && typeof s2.infos === "number";
}
function formatLintAsMarkdown(obj) {
...
`;
    for (const f3 of findings) {
      const location = f3.path ? ` \`${f3.path}\`:` : ":";
      result += `- **${f3.severity}**${location} ${f3.message}
`;
    }
  }
  return result;
}
function formatAsText(data, indent = 0) {
...
` + formatAsText(val, indent + 1) : " " + formatAsText(val, indent + 1);
...
var lint_default = defineCommand({
  ...
  async run({ args }) {
    let content3;
    try {
      content3 = await readInput(args.file);
    } catch (error) {
...
`);
    ...
    const diff = {
      tokens: {
        ...
        components: diffMaps(serializeComponents(beforeReport.designSystem.components), serializeComponents(afterReport.designSystem.components))
      },
      findings: {
...
var FORMATS = ["css-tailwind", "json-tailwind", "tailwind", "dtcg", "css-vars"];
...
`);
    ...
    if (format === "css-tailwind") {
      const handler2 = new TailwindV4EmitterHandler;
      const result = handler2.execute(report.designSystem);
      if (!result.success) {
        console.error(JSON.stringify({ error: result.error.code, message: result.error.message }));
        process.exitCode = 1;
        return;
      }
      process.stdout.write(serializeToCss(result.data.theme));
    } else if (format === "json-tailwind" || format === "tailwind") {
      const handler2 = new TailwindEmitterHandler;
      const result = handler2.execute(report.designSystem);
      if (!result.success) {
        console.error(JSON.stringify({ error: result.error.code, message: result.error.message }));
        process.exitCode = 1;
      ...
      const handler2 = new DtcgEmitterHandler;
      const result = handler2.execute(report.designSystem);
      if (!result.success) {
        console.error(JSON.stringify({ error: result.error.code, message: result.error.message }));
        process.exitCode = 1;
      ...
      const handler2 = new CssVarsEmitterHandler;
      const result = handler2.execute(report.designSystem);
      if (!result.success) {
...
// src/linter/spec-gen/spec-helpers.ts
import { readFileSync as readFileSync4 } from "node:fs";
import { resolve as resolve3, dirname as dirname3 } from "node:path";
import { fileURLToPath as fileURLToPath4 } from "node:url";
function getSpecContent(specPath) {
  if (specPath) {
    return readFileSync4(specPath, "utf-8");
  }
  const currentDir = dirname3(fileURLToPath4(import.meta.url));
  const bundledPath = resolve3(currentDir, "spec.md");
  try {
    return readFileSync4(bundledPath, "utf-8");
  } catch {
    const fallbackPath = resolve3(currentDir, "../linter/spec.md");
    try {
      return readFileSync4(fallbackPath, "utf-8");
    } catch {}
  }
  const devPath = resolve3(currentDir, "../../../../../docs/spec.md");
  try {
    return readFileSync4(devPath, "utf-8");
  } catch {
    throw new Error(`Failed to load spec.md.
` + `  Bundled path: ${bundledPath}
` + `  Dev path:     ${devPath}
` + `If running from a built bundle, ensure the build script copies docs/spec.md into dist/.`);
  }
}
function getRulesTable(rules) {
...
var spec_default = defineCommand({
  meta: {
    name: "spec",
  ...
  async run({ args }) {
    ...
    if (args.format === "json") {
      const jsonOutput = {};
      if (args.rulesOnly) {
        jsonOutput.rules = DEFAULT_RULE_DESCRIPTORS.map((r3) => ({
          name: r3.name,
